...
class RendleyService {
    static get engine() {
        return RendleyStore.Engine.getInstance();
    }
    static init(options) {
    ...
    static deleteAtCurrentTime() {
        ...
        const selectedTransitionId = ApplicationStore.ApplicationStore.selectedTransitionId;
        if (selectedTransitionId != null) {
            const layerId = (_b = RendleyStore.RendleyStore.getLayerFromTransitionId(selectedTransitionId)) === null || _b === void 0 ? void 0 : _b.id;
            if (layerId != null) {
    ...
    static async importSubtitlesSrt(srtContent) {
        const subtitles = RendleyStore.Engine.getInstance().getSubtitlesManager().convertSRTToSubtitles(srtContent);
        return await _a.addSubtitles(subtitles);
    }
    static async addManualSubtitles(textBlocks) {
        const subtitles = new RendleyStore.Subtitles({
    ...
    static async addSubtitles(subtitles) {
        const subtitlesId = RendleyStore.Engine.getInstance().getLibrary().addSubtitles(subtitles);
        const layer = _a.createLayer();
        const subtitlesClip = new RendleyStore.SubtitlesClip({
    ...
    static async createLottieClip(options) {
        ...
        const lottieClip = new RendleyStore.LottieClip({
...
RendleyService.splitAtCurrentTime = () => {
    ...
    const currentTime = RendleyStore.RendleyStore.currentTime;
    const clip = RendleyStore.RendleyStore.clips[selectedClipId];
    if (clip == null) {
        return;
    }
    if ((clip === null || clip === void 0 ? void 0 : clip.type) === RendleyStore.ClipTypeEnum.SUBTITLES) {
        return;
    }
    const layerId = (_b = Object.values(RendleyStore.RendleyStore.layers).find((layer) => layer.clipsIds.includes(selectedClipId))) === null || _b === void 0 ? void 0 : _b.id;
    if (layerId == null) {
        return;
    }
    ...
    for (let i = 0; i < layer.clipsIds.length; i++) {
        if (layer.clipsIds[i] === selectedClipId) {
            if (i > 0) {
                ...
                RendleyStore.RendleyStore.clips[selectedClipId].startTime = layer.clips[selectedClipId].getStartTime();
            }
            else {
                RendleyStore.RendleyStore.clips[selectedClipId].startTime = -layer.clips[selectedClipId].getLeftTrim();
                layer.clips[selectedClipId].setStartTime(RendleyStore.RendleyStore.clips[selectedClipId].startTime);
            }
            break;
        }
    }
...
exports.RendleyService = RendleyService;
